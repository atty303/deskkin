...
pub(crate) fn resource_identity() -> ResourceIdentity {
    ResourceIdentity {
...
fn current_process_owner() -> Option<ProcessOwner> {
...
pub(crate) fn finalize_operation_records(records: &mut [SemanticRecord], outcome: RunOutcome) {
...
impl Recorder {
    ...
    pub(crate) fn from_environment(mode: RecordingMode) -> Self {
    ...
    pub(crate) fn publish(&self, mut run: DiagnosticRun) -> Publication {
    ...
    fn publish_health_best_effort(&self, publication: &Publication) -> bool {
        (|| {
            reject_symlink_if_exists(&self.root)?;
            create_private_dir(&self.root)?;
            let lock = self.open_lock()?;
...
pub(crate) fn new_run_id(prefix: &str) -> String {
...
pub(crate) fn publish_scenario_result(name: &str, bytes: &[u8]) -> io::Result<PathBuf> {
...
mod tests {
    ...
    fn outcome_retention_and_explicit_pin_are_independent() {
        ...
        recorder.publish(run("success-1", RunOutcome::Success, 1));
        let mut pinned = run("success-pinned", RunOutcome::Success, 2);
        pinned.retained = true;
        recorder.publish(pinned);
        recorder.publish(run("success-2", RunOutcome::Success, 3));
        recorder.publish(run("error-1", RunOutcome::Error, 4));
        recorder.publish(run("error-2", RunOutcome::Error, 5));
    ...
    fn in_progress_run_is_recovered_as_partial_crash() {
        ...
        recorder.publish(run("next", RunOutcome::Success, 2));
    ...
    fn publication_uses_private_modes_and_leaves_no_temporary_file() {
        ...
        recorder.publish(run("private", RunOutcome::Success, 1));
        assert_eq!(
            fs::metadata(&root).unwrap().permissions().mode() & 0o777,
            0o700
        );
